...
# 取り残されて lock drift する（#121: v0.9.0 で drift し v1.0.0 リリース作業が
# bump 失敗で詰まった）。alpha-visualizer の release.sh と同じ対処:
# 直後に uv lock を流して、差分が出たら bump commit に amend し、
# タグも同じ commit に貼り直す。
uv lock
