...
`alpha-strike` への貢献を歓迎します。バグ報告・機能提案・ドキュメント改善・コードのいずれでも構いません。本ガイドは開発フローと品質基準をまとめたものです。
...
- **Node.js 22 以上 + pnpm** — フロントエンド開発用（`corepack enable && corepack prepare pnpm@latest --activate` か `brew install pnpm` で導入）
- **Git**
...
新機能・バグ修正には対応するテストを追加してください。バックエンドは `uv run pytest --cov`、フロントエンドは `pnpm run test:ci -- --coverage` で確認できます。
...
将来的に第三言語（中文・韓文等）を追加する際は `STRINGS` map に `zh` / `ko` フィールドを追加するだけで済むよう、徐々に集約することを推奨します（[Issue #151](https://github.com/alforge-labs/alpha-strike/issues/151) で段階移行中）。
...
- **バグ報告・機能要望**: [GitHub Issues](https://github.com/alforge-labs/alpha-strike/issues)
- **質問・議論**: [GitHub Discussions](https://github.com/alforge-labs/alpha-strike/discussions)（有効化されている場合）
