...
|--------|--------------|----------|----------------|------|
| 2026-08-11 | 1.1.0 | 10.9.6908 | 10.9.6918 (`moomoo_OpenD_10.9.6918_Ubuntu18.04`) | 起動時に events ディレクトリへバージョンメタ（`_meta.json`）を書き出す機能 (#131) の反映。SDK / OpenD は不変。**`-U` を付けず `alpha-strike==1.1.0` のみ差し替えた**（`-U` だと fastapi 0.140.0→0.141.1・starlette 1.3.1→1.6.0・uvicorn 0.51.0→0.52.1 等 9 パッケージが `uv.lock`＝CI 検証済みより先行してドリフトするため）。再起動後 `is-active`=active・`/health` ok・`/health/ready` が oanda=ok / moomoo=ok、`_meta.json` に version 1.1.0 が生成されることを確認。Mac 側 `alpha-forge live sync-events` で取得し、alpha-visualizer `/api/versions` の strike 行が status=ok / current=1.1.0 になることまで確認。US 株式休場中（ET 22:25）に実施 |
