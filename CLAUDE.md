...
| `SIGNAL_WATCHDOG_ENABLED` | シグナル途絶の監視。`1`（デフォルト）または `0`。TradingView アラートのサイレント失効を検知して ntfy 通知する。**v1.3.0 以降はサーバープロセス内では動作しない**。`alpha-strike-watchdog` を systemd timer（または cron）から定期実行することが前提 |
